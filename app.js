...
// After loading content, bring the first field (Role) to the top of the
// viewport under the sticky header — not the page top, which shows the hero
// instead of the start of the new content.
function scrollToFirstField() {
  const first = fields[STRING_SLOTS[0]];
  const target = (first && first.closest(".field-card")) || first || el.form;
...
renderSuggestions();
renderExampleChips();
// Restore state BEFORE the first setFormat: setFormat → updatePreview persists
// the form to localStorage, so calling it against a still-empty form would
// overwrite the saved draft before restoreCurrent could read it.
...
// ============================================================
// MODE TABS (single / orchestra / help)
// ============================================================
// Orchestra is a Pro feature; this public bundle only handles view
// visibility and shows a locked teaser inside #orchestrationView.
// The full Orchestra editor lives at app.prompt.phbeks.com.
const MODE_KEY = "context.composer.mode.v1";
const _modeViews = {
  single: document.getElementById("classicView"),
  orchestra: document.getElementById("orchestrationView"),
  tools: document.getElementById("toolsView"),
  context: document.getElementById("contextView"),
  eval: document.getElementById("evalView"),
  help: document.getElementById("helpView"),
};
const _modeMeter = document.querySelector(".slot-meter");
const _modeScoreWrap = el && el.scoreVal ? el.scoreVal.parentElement : null;
const _modeScoreLabel = document.querySelector(".score-chip .score-label");

// Hero title + subtitle change per mode so the heading above each teaser is
// accurate (not always "Prompt Composer"). The <span> is the accent-colored word.
const _heroH1 = document.querySelector(".hero-text h1");
const _heroSub = document.querySelector(".hero-sub");
const _heroCopy = {
  single: {
    h1: "Prompt <span>Composer</span>",
    sub: "Build prompts in a structured R-G-C-B-T-S frame. Learn the technique while you write — local-only, no signup, no tracking.",
  },
  orchestra: {
    h1: "Agent <span>Orchestra</span>",
    sub: "Design multi-agent systems — orchestrator/worker, sequential, parallel, debate, and handoff patterns, with a per-agent editor and runnable export. A Pro feature.",
  },
  tools: {
    h1: "Agent <span>Tool Builder</span>",
    sub: "Compose typed, validated function-calling tool definitions for the Anthropic Messages API — live JSON preview, schema validation, and a reusable library. A Pro feature.",
  },
  context: {
    h1: "Context <span>Pipeline</span>",
    sub: "Budget the context window like RAM — per-source token allocation, eviction triggers, history compaction, and hand-off rules. A Pro feature.",
  },
  eval: {
    h1: "AI <span>Eval</span>",
    sub: "Run your prompt through a meta-LLM — rewrite for clarity, score R-G-C-B-T-S, spellcheck, or code-check — using your own API key. A Pro feature.",
  },
  help: {
    h1: "Prompt <span>Composer</span>",
    sub: "Build prompts in a structured R-G-C-B-T-S frame. Learn the technique while you write — local-only, no signup, no tracking.",
...
function setMode(mode) {
  if (!_modeViews[mode]) mode = "single";
  try { localStorage.setItem(MODE_KEY, mode); } catch {}
  document.querySelectorAll(".mode-tab").forEach((t) => {
    t.classList.toggle("is-active", t.dataset.mode === mode);
  });
  for (const [m, view] of Object.entries(_modeViews)) {
    if (view) view.hidden = (m !== mode);
  }
  const isSingle = mode === "single";
  if (_modeMeter) _modeMeter.style.visibility = isSingle ? "" : "hidden";
  if (_modeScoreWrap) _modeScoreWrap.style.visibility = isSingle ? "" : "hidden";
  if (_modeScoreLabel) {
    _modeScoreLabel.textContent =
      mode === "help" ? "Reference" : (mode === "orchestra" ? "Orchestra" : "Composition");
  }
  const _hero = _heroCopy[mode] || _heroCopy.single;
  if (_heroH1) _heroH1.innerHTML = _hero.h1;
  if (_heroSub) _heroSub.textContent = _hero.sub;
  if (mode === "help") window.scrollTo({ top: 0, behavior: "auto" });
}
...
document.querySelectorAll(".help-jump[data-jump-mode]").forEach((b) => {
  b.addEventListener("click", () => setMode(b.dataset.jumpMode));
});

setMode(localStorage.getItem(MODE_KEY) || "single");

// L1 nav: clicking "Library" must surface the templates band even if the
// user is currently in help or orchestra mode.
document.querySelectorAll('a.topnav-link[href$="#library"]').forEach((a) => {
  a.addEventListener("click", () => setMode("single"));
});

// ---------- mobile nav drawer ----------
// Drawer mode-tabs share class="mode-tab" so the setMode binding above already
// wires them; this just toggles the drawer open/closed and closes on select.
(function () {
  ...
  function setDrawer(open) {
    ...
    // `inert` (not aria-hidden) so a link that keeps focus while the drawer
    // closes doesn't trip the "aria-hidden on a focused ancestor" console error.
    drawer.inert = !open;
  ...
  drawer.querySelectorAll(".mode-tab, a.topnav-link").forEach((el) => {
